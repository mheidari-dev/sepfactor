<?php
namespace YahnisElsts\PluginUpdateCheckerSK\v5p0;
	...
	abstract class Metadata {
		...
		protected static function createFromJson($json, $target) {
			...
			if ( empty($apiResponse) || !is_object($apiResponse) ){
				...
				update_option('salesking_license_debug1', $apiResponse);
				update_option('salesking_license_debug2', $json);
				...
				$failed_attempts = intval(get_option('salesking_failed_license_'.$license, 0));

				if ($failed_attempts >= 1){
					// set as inactive 
					update_option('pluginactivation_'.$email.'_'.$license.'_'.$bottom_host_name, 'inactive');
				} else {
					update_option('salesking_failed_license_'.$license, ($failed_attempts+1));
					update_option('salesking_failed_license_'.$license, ($failed_attempts+1));
				}
				...
				update_option('salesking_failed_license_'.$license, 0);
